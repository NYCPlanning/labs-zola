...
export default class ZoningDistrictFragment extends MF.Fragment {
  @attr('string')
  zonedist;
  ...
  get zoneabbr() {
    const zonedist = this.get('zonedist');
    const abbr = zonedist.match(/\w\d*/)[0].toLowerCase();

    if (zonedist in zoningAbbr) {
      return zoningAbbr[zonedist];
    }
