...
module('Acceptance | navigating to qpd url breaks', function(hooks) {
  ...
  test('Visiting index with QPs directly does not break', async function(assert) {
  ...
  test('Visiting index with QPs of different types does not break', async function(assert) {
    ...
    await percySnapshot('after a refresh, previous QPs are still applied');
    ...
    assert.equal(boroughs.checked, true); // this was turned ON earleir
