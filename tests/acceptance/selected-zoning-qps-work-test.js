...
module('Acceptance | selected zoning qps work', function (hooks) {
  ...
  test('Selected zoning QPs are applied', async function (assert) {
    ...
    await visit(
      `/about?selectedZoning=[${zoning.map(
        (z) => `"${z}"`
      )}]&selectedOverlays=[${overlays.map((o) => `"${o}"`)}]`
    );
  ...
  test('Can click and URL updates', async function (assert) {
    ...
    const { selectedZoning, selectedOverlays } = extractQueryParams(
      currentURL()
    );

    assert.ok(
      !selectedZoning.includes('C8') && !selectedOverlays.includes('C1-1')
    );
  });
