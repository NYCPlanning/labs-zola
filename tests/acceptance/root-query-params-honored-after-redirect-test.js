...
module(
  'Acceptance | root query params honored after redirect',
  function (hooks) {
    ...
    test('visiting site with query params on root are honored after the redirect to about', async function (assert) {
      await visit(
        '/?layer-groups=%5B%22street-centerlines%22%2C%22tax-lots%22%5D'
      );

      assert.equal(
        currentURL(),
        '/about?layer-groups=%5B%22street-centerlines%22%2C%22tax-lots%22%5D'
      );
    });
