...
module('Integration | Component | layer-palette', function (hooks) {
  ...
  test('toggling the Show Land Use Colors checkbox in the layer-palette changes the visibility', async function (assert) {
    ...
    this.layerGroupsObject = this.layerGroups.reduce((accumulator, current) => {
    ...
    assert.equal(
      taxLots.layers.findBy('id', 'pluto-fill').paint['fill-opacity'],
      0.5
    );
  });
  ...
  test('toggling aerials changes the year shown', async function (assert) {
    ...
    assert.equal(
      aerialsLayer.layers.findBy('id', 'aerials-2008').visibility,
      true
    ); // clicking on the new radio button changes the visibility
  });
